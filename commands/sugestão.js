...
exports.run = async (client, message, args) => {
  ...
  .setColor('#a67dff')    
    ...
    const embed = new Discord.MessageEmbed()
        ...
        if (!canal) return message.reply(`esse Discord não possui um canal definido para envio de sugestões. Caso seja um Administrador, defina o canal utilizando a sintaxe: \`!setsugestão\``)
              
  
        canal.send(embed)
        .then(function (msg) {
            msg.react("782645875276513320"); // like
            msg.react("782646065160781854"); // deslike 
